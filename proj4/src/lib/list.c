...
inline ListNode getHeadNode(List list) {
    assert(list != NULL);
    if (isEmptyList(list))return NULL;
...
void concatList(List dst, List src) {
    ...
    ListNode srcHead = src->leader->next;
    dst->tail->next = srcHead;
    srcHead->prev = dst->tail;
    dst->tail = src->tail;
    dst->size += src->size;
    freeNode(src->leader);
    free(src);
...
bool existsInList(List list, void *data) {
    for (ListNode node = getHeadNode(list); node; node = node->next) {
