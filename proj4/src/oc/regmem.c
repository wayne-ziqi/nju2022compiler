...
static inline RegKind allocate_reg(VarInfo var, InterCode code, Block block) {
    ...
    // find the reg whose first appear is the latest
    RegKind best = REG_S0;
    int distance = (int) 0x80000000;
    int dist_recd[8] = {-1, -1, -1, -1, -1, -1, -1, -1};
    for (int i = REG_S0; i <= REG_S7; ++i) {
        VarInfo reg_var = regInUse[i];
        bool found = false;
        for (int j = code->offset; j < sizeArray(block->codes); ++j) {
            ...
            free_Array(uses);
            if (k != cnt_uses && dist_recd[i - REG_S0] == -1) {
                dist_recd[i-REG_S0] = j;
                found = true;
                break;
            }
            if (found)break;
    ...
    for (int i = 0; i < 8; ++i) {
        if (dist_recd[i] > distance){
            distance = dist_recd[i];
            best = i + REG_S0;
        }
    }
