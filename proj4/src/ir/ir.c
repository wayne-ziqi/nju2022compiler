...
inline char *operand_to_string(Operand o) {
    switch (o->kind) {
        case OPD_LABEL: {
...
InterCode genIR_fetchAddr(Operand left, Operand addr) {
    assert(left && addr);
    InterCode code = new_code(CODE_LOADADDR);
    code->opd_num = 2;
    code->twin.o1 = left;
    code->twin.o2 = addr;
    return code;
...
Operand getDefOf(InterCode code) {
    if (code->kind == CODE_PARAM || code->kind == CODE_READ)
        return code->single.o;
    else if (code->kind == CODE_ADD || code->kind == CODE_SUB ||
             code->kind == CODE_MUL || code->kind == CODE_DIV
            ) {
        return code->triple.o3;
    } else if (code->kind == CODE_GETADDR || code->kind == CODE_LOADADDR ||
               code->kind == CODE_CALL || code->kind == CODE_ASSIGN ||
               code->kind == CODE_DEC
            ) {
        return code->twin.o1;
    }
...
Array getUseOf(InterCode code) {
    Array ret = new_Array();
    if (code->kind == CODE_ARG || code->kind == CODE_RETURN ||
        code->kind == CODE_WRITE) {
        Operand o = code->single.o;
        if (o->kind == OPD_VAR || o->kind == OPD_TMP)
            appendArray(ret, o);
    } else if (code->kind == CODE_ASSIGN || code->kind == CODE_STOREADDR || code->kind == CODE_LOADADDR
            ) {
        Operand o2 = code->twin.o2;
        if (o2->kind == OPD_VAR && code->kind != CODE_LOADADDR || o2->kind == OPD_TMP)
            appendArray(ret, o2);
        if (code->kind == CODE_STOREADDR) { // store destination is also regarded as used
            Operand o1 = code->twin.o1;
            appendArray(ret, o1);
        }
    } else if (code->kind == CODE_ADD || code->kind == CODE_SUB ||
        ...
        Operand o1 = code->triple.o1;
        Operand o2 = code->triple.o2;
        if (o1->kind == OPD_VAR || o1->kind == OPD_TMP) appendArray(ret, o1);
        if (o2->kind == OPD_VAR || o2->kind == OPD_TMP) appendArray(ret, o2);
    } else if (code->kind == CODE_IF) {
        Operand o1 = code->if_code.cond1;
        Operand o2 = code->if_code.cond2;
        if (o1->kind == OPD_VAR || o1->kind == OPD_TMP) appendArray(ret, o1);
        if (o2->kind == OPD_VAR || o2->kind == OPD_TMP) appendArray(ret, o2);
    }
    return ret;
