 ...
 * @return
 */
Operand exp_simplifier(TNode *exp, List *code);
...
List irt_Stmt(TNode *stmt) {
    ...
    if (child1->value_t == CompSt_) {
        ...
        List code1;
        Operand t = exp_simplifier(child1->next_sibling, &code1);
...
List irt_binary_solver(TNode *exp1, TNode *exp2, CodeKind bop, Operand place) {
    List code1;
    List code2;
    Operand t1 = exp_simplifier(exp1, &code1);
    Operand t2 = exp_simplifier(exp2, &code2);
...
List irt_Args(TNode *args, List arg_list) {
    ...
    for (TNode *child = args->first_child; child; child = child->next_sibling) {
        if (child->value_t == Exp_) {
            List code1;
            Operand t = exp_simplifier(child, &code1);
            if (t->kind == OPD_VAR) {
    ...
    if (child1->value_t == INT_) {
        ...
        if (child2->value_t == ASSIGNOP_) {
            ...
            if (id->value_t == ID_) {
                ...
                List code1;
                Operand t = exp_simplifier(child2->next_sibling, &code1);
                // for assignment between array and structures
                if (t->kind == OPD_VAR && var->type->kind != TYPE_BASIC) {
                    ...
                    if (equalType(var->type, rightInfo->type)) {
                        ...
                        pushTail(code1, newListNode(genIR_getAddr(new_OpVar(var->name), t)));
                        if (place)
                            pushTail(code1, newListNode(genIR_assign(place, new_OpVar(var->name))));
                        return code1;
                    }
                }
                pushTail(code1, newListNode(genIR_assign(new_OpVar(var->name), t)));
                if (place)
                    pushTail(code1, newListNode(genIR_assign(place, new_OpVar(var->name))));
                return code1;
            } else {
...
List irt_CondExp(TNode *condExp, Operand label_true, Operand label_false) {
    ...
    if (child1->value_t == NOT_)
        ...
        if (child2->value_t == RELOP_) {
            List code1;
            List code2;
            Operand t1 = exp_simplifier(child1, &code1);
            Operand t2 = exp_simplifier(child3, &code2);
