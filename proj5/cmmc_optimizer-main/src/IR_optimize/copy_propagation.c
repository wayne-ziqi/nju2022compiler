...
static void CopyPropagation_teardown(CopyPropagation *t) {
    for_map(IR_block_ptr, Fact_def_use_ptr, i, t->mapInFact)RDELETE(Fact_def_use, i->val);
    for_map(IR_block_ptr, Fact_def_use_ptr, i, t->mapOutFact)RDELETE(Fact_def_use, i->val);
...
CopyPropagation_meetInto(CopyPropagation *t,
    ...
    for_map(IR_var, IR_var, it, target->def_to_use)
        if (!VCALL(fact->def_to_use, exist, it->key) ||
            VCALL(fact->def_to_use, get, it->key) != it->val) {
...
void CopyPropagation_print_result(CopyPropagation *t, IR_function *func) {
    ...
    for_list(IR_block_ptr, i, func->blocks) {
        ...
        for_map(IR_var, IR_var, j, in_fact->def_to_use)printf("{v%u := v%u} ", j->key, j->val);
        printf("\n");
        printf("[Out(top:%d)]: ", out_fact->is_top);
        for_map(IR_var, IR_var, j, out_fact->def_to_use)printf("{v%u := v%u} ", j->key, j->val);
        printf("\n");
        printf("=================\n");
    }
}
