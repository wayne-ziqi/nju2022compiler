...
static CPValue meetValue(CPValue v1, CPValue v2) {
    ...
    if (v1.kind == NAC || v2.kind == NAC)
        ...
        return get_UNDEF();
    else if ((v1.kind == CONST && v2.kind == UNDEF) || (v1.kind == UNDEF && v2.kind == CONST))
...
static void ConstantPropagation_teardown(ConstantPropagation *t) {
    for_map(IR_block_ptr, Map_ptr_IR_var_CPValue, i, t->mapInFact)RDELETE(Map_IR_var_CPValue, i->val);
    for_map(IR_block_ptr, Map_ptr_IR_var_CPValue, i, t->mapOutFact)RDELETE(Map_IR_var_CPValue, i->val);
...
ConstantPropagation_meetInto(ConstantPropagation *t,
    ...
    bool updated = false;
    for_map(IR_var, CPValue, i, *fact)updated |= Fact_meet_value(target, i->key, i->val);
    return updated;
}
