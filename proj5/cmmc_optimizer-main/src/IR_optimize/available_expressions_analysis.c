...
static void AvailableExpressionsAnalysis_teardown(AvailableExpressionsAnalysis *t) {
    for_map(IR_block_ptr, Fact_set_var_ptr, i, t->mapInFact)RDELETE(Fact_set_var, i->val);
    for_map(IR_block_ptr, Fact_set_var_ptr, i, t->mapOutFact)RDELETE(Fact_set_var, i->val);
    for_map(IR_var, Vec_ptr_IR_var, i, t->mapExprKill)DELETE(i->val);
...
void AvailableExpressionsAnalysis_transferStmt(AvailableExpressionsAnalysis *t,
    ...
    if (def != IR_VAR_NONE && VCALL(t->mapExprKill, exist, def)) {
        ...
        for_vec(IR_var, i, *killed_expr_var)VCALL(fact->set, delete, *i);
    }
    // e_gen
...
void AvailableExpressionsAnalysis_print_result(AvailableExpressionsAnalysis *t, IR_function *func) {
    ...
    for_list(IR_block_ptr, i, func->blocks) {
        ...
        for_set(IR_var, var, in_fact->set)printf("v%u ", var->key);
        printf("\n");
        printf("[Out(top:%d)]: ", out_fact->is_top);
        for_set(IR_var, var, out_fact->set)printf("v%u ", var->key);
        printf("\n");
        printf("=================\n");
    }
}
