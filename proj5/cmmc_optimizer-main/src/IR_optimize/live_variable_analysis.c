...
static void LiveVariableAnalysis_teardown(LiveVariableAnalysis *t) {
    for_map(IR_block_ptr, Set_ptr_IR_var, i, t->mapInFact)DELETE(i->val);
    for_map(IR_block_ptr, Set_ptr_IR_var, i, t->mapOutFact)DELETE(i->val);
...
void LiveVariableAnalysis_transferStmt(LiveVariableAnalysis *t,
    ...
    for (unsigned i = 0; i < use.use_cnt; i++) {
        ...
        if (!use_val.is_const) {
            ...
            VCALL(*fact, insert, use_var); // kill/gen ?
        }
    }
}
...
void LiveVariableAnalysis_print_result(LiveVariableAnalysis *t, IR_function *func) {
    ...
    for_list(IR_block_ptr, i, func->blocks) {
        ...
        printf("[In]:  ");
        for_set(IR_var, var, *in_fact)printf("v%u ", var->key);
        printf("\n");
        printf("[Out]: ");
        for_set(IR_var, var, *out_fact)printf("v%u ", var->key);
        printf("\n");
        printf("=================\n");
    }
}
