...
static inline unsigned int hash_mod(UInt key, unsigned int slotNumber) {
    return hash_uint(key->u) % slotNumber;
}
...
static inline unsigned int getIndex(void *key, KeyType key_t, unsigned int slotNumber) {
    ...
    if (key_t == STRING)
        ...
        index = hash_mod((UInt) key, slotNumber);
    } else
...
List getHashAll(HashMap hashMap) {
    List ret = new_list();
    for (int i = 0; i < hashMap->slot_num; ++i) {
        for (ListNode node = getHeadNode(hashMap->keySet[i]); node; node = node->next) {
            pushTail(ret, newListNode(node->data));
        }
    }
    return ret;
}
