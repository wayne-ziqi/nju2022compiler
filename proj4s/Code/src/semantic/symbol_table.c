...
ScopeList new_ScopeList() {
    ...
    if(isEmptyStack(ScopeStack__)) sm_list->father_scope = NULL;
    else{
        sm_list->father_scope = getTop(ScopeStack__);
    }
...
SymbolInfo getInfo(const char *name) {
    ...
    while (curScope != NULL) {
        ...
        curScope = curScope->father_scope;
    }
    return NULL;
}
...
bool searchInfoRulePLat(const char *name, SymbolInfo info) {
    return strcmp(name, info->name) == 0;
}

SymbolInfo getInfoFlat(const char *name) {
    return getFirstOf(SymbolTable__, (void *) name,
                      (void *) name, (bool (*)(void *, void *)) searchInfoRulePLat
    );
}
