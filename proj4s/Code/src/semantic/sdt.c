...
void initIRReadWrite() {
    SymbolInfo funRead = new_SymbolInfo(
            "read", genBasic(BASIC_INT),
            SYM_FUNC, new_FuncDetail(true, -1)
    );
    SymbolInfo funWrite = new_SymbolInfo(
            "write", genBasic(BASIC_INT),
            SYM_FUNC, new_FuncDetail(true, -1)
    );
    pushTail(funWrite->funDetail->paramsType, newListNode(genBasic(BASIC_INT)));
    appendArray(funWrite->funDetail->paramsName, "value");
    addInfo(funRead), addInfo(funWrite);
}

int do_sdt() {
//    show_tree(AST__, 0);
    initSymbolSystem();
    // prepare for generalize IR
    initIRReadWrite();
...
void resolve_ExtDefList(TNode *node) {
    ...
    int num_child = sizeArray(children);
...
Type resolve_Specifier(TNode *specifier) {
    ...
    if (specKind->value_t == TYPE_) {
        ...
        int decSize = sizeArray(specArray);
